...
- `inventoryApi` 已配置封面关闭及用户/全局日额度；`quickEntryApi` 已配置文字 AI、请求超时及用户/全局日额度，不存在 STT/OCR 环境变量。
...
| `inventoryApi` | 60 秒 | Node.js 20.19 | 封面关闭，额度已配置 |
...
2. 微信公众平台 → 设置与开发 → 服务内容声明 → 用户隐私保护指引：删除录音、语音识别、相机/OCR 相关声明；保留并写明“用户输入的物品文字会发送至大模型用于结构化解析”。
