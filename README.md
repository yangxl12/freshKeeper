...
- 我的：弹窗式提醒设置、反馈建议、已用完处理记录，以及支持恢复、手动或批量彻底删除的回收站
...
用户提交的建议保存在当前微信云开发环境的 `user_feedback` 集合中。上线前需在云开发控制台创建该集合并把权限设为“仅管理员可读写”；管理员可在“云开发 → 数据库 → user_feedback”查看和更新 `status`。
