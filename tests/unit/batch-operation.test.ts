...
describe('批量操作 → 回收站范围', () => {
  ...
  it('翻多页时按档位下发，不把已加载数组反复重传', async () => {
    ...
    expect(listTrashMock).toHaveBeenCalledTimes(7)
    ...
    expect(page.data.scopeLabel).toBe('回收站全部 · 200 件')
    expect(page.data.errorMessage).toContain('最多加载 200 项')
  })
